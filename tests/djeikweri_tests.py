...
def test_element():
	...
	ok( jQuery("*").size() >= 30, "Select all_nodes" );
	all_nodes = jQuery("*"); good = True;
...
if ( location.protocol != "file:" ) :
	def test_XML_Document_Selectors():
		expect(7);
		stop();
		with Djeikweri(join(dirname(__file__), "data/with_fries.xml")) as xml :
...
def test_id():
	...
	t( "ID selector using UTF8", u"#台北Táiběi", [u"台北Táiběi"] );
	t( "Multiple ID selectors using UTF8", u"#台北Táiběi, #台北", [u"台北Táiběi",u"台北"] );
	t( "Descendant ID selector using UTF8", u"div #台北", [u"台北"] );
	t( "Child ID selector using UTF8", u"form > #台北", [u"台北"] );
	...
	a = jQuery('<div><a name="tName1">tName1 A</a><a name="tName2">tName2 A</a><div id="tName1">tName1 Div</div></div>').appendTo('#main');
...
def test_class():
	...
	t( "Class selector using UTF8", u".台北Táiběi", ["utf8class1"] );
	##t( "Class selector using UTF8", u".台北", ["utf8class1","utf8class2"] );
	t( "Class selector using UTF8", u".台北Táiběi.台北", ["utf8class1"] );
	t( "Class selector using UTF8", u".台北Táiběi, .台北", ["utf8class1","utf8class2"] );
	t( "Descendant class selector using UTF8", u"div .台北Táiběi", ["utf8class1"] );
	t( "Child class selector using UTF8", u"form > .台北Táiběi", ["utf8class1"] );
...
##
## Tests from here on unsupported, some of them fail
##
...
#
#	a = jQuery('<div><a id="tName1ID" name="tName1">tName1 A</a><a id="tName2ID" name="tName2">tName2 A</a><div id="tName1">tName1 Div</div></div>').appendTo('#main').children();
#
...
#
#
#def test_pseudo_minus_colon_not():
#	expect(24);
...
#
#def test_pseudo_minus_position():	
#	expect(25);
...
#
#def test_pseudo_minus_visibility():
#	expect(11);
#
...
#
#	div_tags = jQuery('<div/>').appendTo("body");
#	div_tags.css({ fontSize: 0, lineHeight: 0 });## IE also needs to set font-size and line-height to 0
#	div_tags.width(1).height(0);
#	t( "Is Visible", '#nothiddendivchild:visible', ['nothiddendivchild'] );
#	t( "Is Not Visible", '#nothiddendivchild:hidden', [] );
#	div_tags.width(0).height(1);
#	t( "Is Visible", '#nothiddendivchild:visible', ['nothiddendivchild'] );
#	t( "Is Not Visible", '#nothiddendivchild:hidden', [] );
#	div_tags.width(1).height(1);
#	t( "Is Visible", '#nothiddendivchild:visible', ['nothiddendivchild'] );
#	t( "Is Not Visible", '#nothiddendivchild:hidden', [] );
#	div_tags.remove();
#
#def test_pseudo_minus_form():
#	expect(8);
#
